import type { TrustedByLogo } from "../../components/sections/hero/TrustedByLogos";

export const trustedByLogos: TrustedByLogo[] = [
  {
    id: "databricks",
    name: "Databricks",
    src: "/images/home/companies/databricks.png",
  },
  {
    id: "google-cloud",
    name: "Google Cloud",
    src: "/images/home/companies/google-cloud.png",
  },
  {
    id: "alteryx-1",
    name: "Alteryx",
    src: "/images/home/companies/alteryx.png",
  },
  { id: "ui-path-1", name: "UiPath", src: "/images/home/companies/ui-path.png" },
  { id: "figma", name: "Figma", src: "/images/home/companies/figma.png" },
  { id: "aws", name: "AWS", src: "/images/home/companies/aws.png" },
  {
    id: "alteryx-2",
    name: "Alteryx",
    src: "/images/home/companies/alteryx.png",
  },
  { id: "ui-path-2", name: "UiPath", src: "/images/home/companies/ui-path.png" },
];
